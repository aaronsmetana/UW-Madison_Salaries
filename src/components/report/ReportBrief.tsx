...
export function ReportBrief({ model, hovered, onHover }: {
  ...
  const noteDefs: { id: string; when: boolean; text: ReactNode }[] = [
    ...
    { id: 'raiseCycle', when: !!raiseCycle, text: "The raise-cycle comparison covers continuing raises only: people with one appointment in both snapshots, in the same job code, at the same appointment percentage and pay basis — a promotion, an FTE change or a change in how pay is reported is not counted as a raise. The annualized rate compounds the median cycle raise over the actual elapsed months between the two snapshots. Observed cycle raises include the Legislature's Joint Committee on Employment Relations (JCOER)-approved pay plan adjustments, which are applied broadly and so maintain rather than close relative pay gaps." },
            ...
            <Paper p="xl" bg="var(--mantine-color-accent-light)" mb="lg">
              ...
              {showReceipt && (
                <Box mt="lg" pt="md" style={{ borderTop: '1px solid var(--mantine-color-default-border)' }}>
                  <Eyebrow mb={6}>How this figure is built</Eyebrow>
                  <Stack gap={4}>
                    {receipt.map((line) => {
                    ...
                    {rows.map((r) => {
                      ...
                      return (
                        ...
                        >
                          <Table.Td>
                            {r.isSubject ? (
                              <><b>{r.name}</b> <Badge size="xs" variant="light" color="accent" ml={4}>Review Subject</Badge></>
                            ) : (
                              <>
                                {anonymize ? (
                                ...
                                {r.isAnomaly
                                  ? <Badge size="xs" variant="filled" color="accent" ml={6}>Pay inversion</Badge>
                                  : r.lessTenure && <Badge size="xs" variant="light" color="accent" ml={6}>less tenure</Badge>}
                              </>
                            )}
                          </Table.Td>
