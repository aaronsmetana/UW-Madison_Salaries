...
/**
 * A run is "the same appointment, as far as the matcher can follow it". Each test owns one property,
 * and the reused-lane case is asserted directly because it is the one the whole distinction exists for.
 */
describe('appointment runs', () => {
  const other = (snapshotId: string, jobCode: string, department: string, pay: number, fte: number): Row => ({
    snapshotId, jobCode, school: L_AND_S, department, fte, pay,
  });

  it('gives rows chained by priorOf one run', () => {
    const m = matchAppointments(glowacki, get);
    expect(m.run.get(glowacki[2])).toBe(m.run.get(glowacki[0]));
    expect(m.run.get(glowacki[3])).toBe(m.run.get(glowacki[1]));
    expect(m.run.get(glowacki[0])).not.toBe(m.run.get(glowacki[1]));
  });

  /**
   * The Nov 2021 TTC shape: one lane, two runs. `pre` and `post` share no job code, so nothing pairs
   * across them and both lanes restart — the SAME lane numbers, new appointments as far as the source
   * can tell. Keying anything on the lane would join them.
   */
  it('starts a new run when a lane number is reused', () => {
    const rows = [
      lecturer('2021-11-pre', L_AND_S, GERMAN, 40079),
      lecturer('2021-11-pre', INTL, SLAVIC, 16650),
      other('2021-11-post', 'TL999', GERMAN, 40079, 0.5),
    ...
    const m = matchAppointments(rows, get);
    // Same lane numbers on both sides of the boundary...
    expect([m.lane.get(rows[0]), m.lane.get(rows[2])]).toEqual([1, 1]);
    // ...and different runs, because nothing connects them.
    expect(m.run.get(rows[2])).not.toBe(m.run.get(rows[0]));
    expect(new Set(rows.map((r) => m.run.get(r))).size).toBe(4);
  });

  it('puts a row with no partner anywhere in a run of its own', () => {
    const rows = [...glowacki, other('2022-03', 'AD006', 'Language Institute', 0, 0.00025)];
    const m = matchAppointments(rows, get);
    const mine = m.run.get(rows[4]);
    expect(rows.filter((r) => m.run.get(r) === mine)).toEqual([rows[4]]);
  });
});
